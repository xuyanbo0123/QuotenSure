class RevenuesController < ApplicationController
  ...
  # GET /vantage_media
  def vantage_media
    revenue = Revenue.new(:r_type => '_click', :source => 'VantageMedia', :amount => params[:rate], :token => params[:tid])
    revenue.token = revenue.token.insert(20, '-').insert(16, '-').insert(12, '-').insert(8, '-')
    revenue.save
  ...
  #GET /revi_media
  def revi_media
    buyer_token = params[:TransactionId]
    unless buyer_token.blank?
      lead = Lead.find_by_buyer_token(buyer_token)
      lead.update_revi_revenue(params[:Result], params[:Payout])
    end
  ...
  def revenue_params
    params.require(:revenue).permit(:r_type, :source, :token, :amount)
  end
end
