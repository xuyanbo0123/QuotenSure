...
////////////////////////////*APPLICATION WIZARD*/////////////////////////
    ...
    function indicateErrorSelectBox() {
        if (newLead.valid() == false)
        {
            newLead.find('select.error').each(
                function(e){
                    $(this).closest('.form-group').find('span.selectboxit.select').addClass('error');
                }
            );
            newLead.find('select.error').on('change', function(e){
                $(this).closest('.form-group').find('span.selectboxit.select').removeClass('error');
    ...
    function initializeSelectBoxIt(tab) {
        tab.find('select').selectBoxIt(selectboxit_settings);
    }
    ...
    function initializeDriver(driver, isPrimary) {
        initializeSelectBoxIt(driver);
        initializeDOB(driver);
    ...
    function showRelationship(driver, isPrimary) {
        ...
        select_relationship.data('selectBox-selectBoxIt').refresh();
    }
    function initializeDOB(tab) {
        //date picker
    ...
    function initializePolicy() {
        initializeSelectBoxIt(policy);
        initializeEXP(policy);
        refreshPolicy();
    ...
    function refreshPolicy() {
        ...
        if (text == 'YES')
        {
            ...
            policy.find('select.company-id').data('selectBox-selectBoxIt').refresh();
    ...
    function initializeEXP(tab) {
        //date picker
        d = tab.find('input.date_picker');
        d.datepicker({
            changeMonth: true,
            changeYear: true,
            yearRange: "2015:2025",
    ...
    function initializeHasIncident() {
        select_has_incident.selectBoxIt(selectboxit_settings);
    ...
    function initializeIncident(incident) {
        initializeSelectBoxIt(incident);
    ...
    function refreshIncident(incident) {
        ...
        select_driver.html("<option value = ''>Please select</option>");
        var isRemoved = true;
        if (selected_id == null) {
            isRemoved = false;
            selected_id = '0';
        ...
        select_driver.data('selectBox-selectBoxIt').refresh();
        if (isRemoved)
    ...
    function showSelectError(select_obj)
    {
       select_obj.closest('.form-group').find('span.selectboxit.select').addClass('error');
    }
    //finalize
    function finalize() {
        finalizeHasIncident();
